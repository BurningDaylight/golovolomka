...
public:	
	...
	figure_t();
	figure_t(const coord3D_t size);
	figure_t(const char filler, const coord3D_t size, const char* data);
	figure_t(const char filler, const coord3D_t size, const coord3D_t* coords, int q);
	
	~figure_t();

	static const figure_t& get_null_figure() { return m_null_figure; }
	
	coord3D_t get_size() const { return m_size; }
	char		get_filler() const { return m_filler; }
	
	void copy(const figure_t& figure);
	
	bool has_solid_line(const coord3D_t& start, axis_t axis) const;
	//bool get_projection(axis_t axis) const;
	
	coord3D_t* get_coords_of_subfigure(int& size, char filler = 0) const; // 0 == any
	...
	bool try_place_figure(const figure_t& figure, const coord3D_t coords);
	bool remove_figure(const figure_t& figure, const coord3D_t coords);
